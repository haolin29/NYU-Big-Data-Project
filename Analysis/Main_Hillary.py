			...
			try:
				...
				try:
					...
					with open('hilary.csv', 'a') as f:
						writer = csv.writer(f)
						writer.writerow([decoded['user']['id_str'],contestant,sentiment_result,tweetLocation,decoded['created_at'].encode('ascii', 'ignore'),decoded['text'].encode('ascii', 'ignore'),decoded['source'].encode('ascii', 'ignore')])
